...
const lookupStar = async () => {
  const instance = await StarNotary.deployed();
  const id =  parseInt(document.getElementById("starId2").value);
  console.log('idToNumber ==' + id )
  let r = await instance.lookUptokenIdToStarInfo(id, {from: account}) ;
...
const App = {
  ...
  setStartInfo: function (message) {
    const status = document.getElementById('startInfo')
    status.innerHTML = message
  },
